...
## Дашборд: «Настройки» не открываются

| Симптом | Решение |
|---------|---------|
| Кнопка «Настройки» ничего не делает | **Ctrl+F5** (жёсткое обновление). В консоли браузера (F12) не должно быть `Unexpected token` в `settings-modal.mjs` |
| Тост «Настройки не загрузились» | Тот же сброс кэша; перезапустите `npm run dashboard` |
| После правок UI | `npm run check:dashboard` и при запущенном дашборде `npm run test:dashboard-settings` |

Типичная причина — синтаксическая ошибка в `dashboard/public/*.mjs` (например незакрытый JSDoc `/** … */` перед `export`). CI и `check:dashboard` ловят это до релиза.
