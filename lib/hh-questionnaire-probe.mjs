...
export async function collectBestQuestionnaire(page, opts = {}) {
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    if (merged.length > prevLen) {
      ...
      stableRounds = 0;
      log(`[probe] шаг ${step}: всего ${merged.length} вопр. (+${merged.length - prevLen}), textarea: ${taVisible}`);
    ...
    const canGrow = taVisible > merged.length;

    if (merged.length > 0 && stableRounds >= 2 && !canGrow) {
