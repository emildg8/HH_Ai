...
const letterStructureRules = `
Структура каждого письма (4–7 предложений, 450–1200 символов):
1) приветствие + зачем именно эта роль (1 фраза из вакансии, не общие слова);
2) 2–3 доказательства из резюме с цифрами/метриками, привязанные к требованиям вакансии;
3) что сделаешь в первые недели / какой формат работы комфортен (1 фраза);
4) мягкое закрытие без канцелярита («буду рад обсудить» / «готов ответить на вопросы»).`;
...
Самопроверка перед ответом: в каждом письме есть (а) название роли или компании, (б) минимум 2 цифры/метрики из резюме, (в) минимум 1 формулировка из требований вакансии.
...
Письма звучат как живой профессионал mid/senior в IT-инфраструктуре и поддержке, не как шаблон ChatGPT.
Если даны эталоны — имитируй только стиль. Только JSON {"variants":[...]}, без текста до/после.`,
  ...
  try {
    const result = await runCoverLetterLlmRound(payloadBase, hasOR, skipOpenRouter, allowCustomFallback);
    result.variants = rankLetterVariants(result.variants, record, desc);
    return result;
  } catch (e) {
...
export async function generateCoverLetterVariants(record, cvBundle) {
  ...
  if (isCoverLetterTwoPhaseEnabled()) {
    try {
      const brief = await generateMatchingBrief(record, cvBundle, desc, { hasOR, skipOpenRouter });
      context.briefBlock = formatBriefForPrompt(brief);
    } catch (e) {
