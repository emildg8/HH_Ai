...
/** @returns {string|null} */
export function checkApplyRateLimits() {
  ...
  if (countApplyLaunchesLastHour() >= maxApplyHour) {
    return `Слишком частые отклики: максимум ${maxApplyHour} в час.`;
  }
  if (countApplyLaunchesLastDay() >= maxApplyDay) {
    return `Дневной лимит откликов: ${maxApplyDay}. Продолжите завтра.`;
  }
  if (countApplyLaunchesLastMonth() >= maxApplyMonth) {
    return `Лимит за 30 дней: ${maxApplyMonth}.`;
  }
  return null;
}
