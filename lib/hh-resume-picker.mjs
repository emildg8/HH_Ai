...
export function questionnaireStepResumeBlocksSubmit(opts = {}) {
  ...
  // Пустой title при известном ideal — не silent pass (не прочитали sticky).
  if (!curTitle) {
    if (ideal && ['devops', 'infra', 'support', 'support_lead', 'tam'].includes(ideal)) {
      return { block: true, reason: 'empty-resume-title-on-quiz' };
    }
    return { block: false, reason: '' };
  }

  const guessed = inferResumeRoleFromHhTitle(curTitle);
  // Sticky devops↔infra — до title-ok (preferred title не маскирует чужой CV vs ideal).
