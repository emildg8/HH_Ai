...
import { getExportableFilePaths } from './settings-registry.mjs';
import { expandDotPatch, getAtPath } from './settings-paths.mjs';
...
export function patchDashboardPreferences(patch) {
  ...
  if (patch.interviewCopilot && typeof patch.interviewCopilot === 'object') {
    prefs.interviewCopilot = { ...(prefs.interviewCopilot || {}), ...patch.interviewCopilot };
    updated.interviewCopilot = prefs.interviewCopilot;
  }
...
export function pickDashboardPrefsExport(prefs = loadPreferences()) {
  ...
  const flat = {};
  for (const path of getExportableFilePaths()) {
    const v = getAtPath(prefs, path);
    if (v !== undefined) flat[path] = v;
  }
  for (const key of DASHBOARD_PREF_KEYS) {
    if (key in prefs && !(key in flat)) flat[key] = prefs[key];
  }
  if (prefs.interviewCopilot && typeof prefs.interviewCopilot === 'object') {
    for (const [k, v] of Object.entries(prefs.interviewCopilot)) {
      const p = `interviewCopilot.${k}`;
      if (!(p in flat)) flat[p] = v;
    }
  }
  return expandDotPatch(flat);
}
...
export function sanitizeDashboardPrefsImport(raw) {
  ...
  const flat = {};
  for (const path of getExportableFilePaths()) {
    const v = getAtPath(nested, path);
    if (v !== undefined) flat[path] = v;
  }
  for (const key of DASHBOARD_PREF_KEYS) {
    if (key in nested && !(key in flat)) flat[key] = nested[key];
  }
  if (nested.interviewCopilot && typeof nested.interviewCopilot === 'object') {
    for (const [k, v] of Object.entries(
      /** @type {Record<string, unknown>} */ (nested.interviewCopilot)
    )) {
      const p = `interviewCopilot.${k}`;
      if (!(p in flat)) flat[p] = v;
    }
