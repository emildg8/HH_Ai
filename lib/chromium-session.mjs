...
export async function launchPersistentContextSafe(profile, launchOpts, meta) {
  ...
  // HH_CDP_ENDPOINT — подключиться к уже запущенному браузеру вместо своего.
  const cdp = String(process.env.HH_CDP_ENDPOINT || '').trim();
  if (cdp) {
    const browser = await chromium.connectOverCDP(cdp);
    const ctx = browser.contexts()[0];
    if (!ctx) throw new Error('CDP: нет контекста в запущенном браузере');
    ctx.__hhCdp = true;
    return ctx;
  }
  ...
  const mergedOpts = {
    ...
    args: [
      ...
      '--no-first-run',
      ...(String(process.env.HH_WINDOW_OFFSCREEN || '').trim() === '1'
...
export async function closeContextSafe(ctx, owner) {
  if (ctx && ctx.__hhCdp) return; // общий браузер — не закрываем
  try {
