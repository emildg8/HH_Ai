...
/**
 * Волна дня: applied за сутки + кандидаты с письмом (для point gate).
 * @param {object[]} records
 * @param {{ now?: Date, focusId?: string, focusLetter?: string, focusCompany?: string }} [opts]
 */
export function collectPointDayWaveLetterItems(records, opts = {}) {
  ...
  for (const rec of records || []) {
    ...
    const appliedAt = rec.hhApply?.appliedAt || rec.appliedAt || rec.coverLetter?.approvedAt;
    const appliedMs = appliedAt ? Date.parse(String(appliedAt)) : NaN;
    const appliedToday =
      status === 'applied' && Number.isFinite(appliedMs) && appliedMs >= dayMs;
    const pendingWithLetter = status === 'pending' || status === 'approved';
    if (appliedToday || pendingWithLetter || id === opts.focusId) {
      byId.set(id, {
        id,
  ...
  if (opts.focusId && opts.focusLetter) {
    byId.set(opts.focusId, {
      id: opts.focusId,
      company: String(opts.focusCompany || byId.get(opts.focusId)?.company || '').slice(0, 40),
