...
/**
 * Флаги «чего избегать» при gen/inject по уже отправленным/shortlist письмам волны.
 * @param {Array<string|{ letter?: string }>} letters
 */
export function waveMetricAvoidFlags(letters) {
  const texts = (letters || [])
    .map((x) => (typeof x === 'string' ? x : String(x?.letter || '')))
    .filter((t) => t.trim());
  return {
    avoidMttr15: texts.some((t) => letterHasMttr15Fingerprint(t)),
    avoidItOneOpen: texts.some((t) => letterHasSameItOneOpeningFingerprint(t)),
    n: texts.length,
  };
}

/**
 * Заменить каскад −15%/MTTR на коммерческий факт **без укорочения** текста.
 * @param {string} letter
 * @returns {string}
 */
export function rewriteMttr15WithoutShorten(letter) {
  const t = String(letter || '');
  if (!letterHasMttr15Fingerprint(t)) return t;
  const alt =
    'До июня 2026 в Softline сопровождал внутренние Linux-контуры: доступы, регламенты и мониторинг.';
  let out = t
    .replace(
      /(?:время\s+реакц\w*|MTTR)[^.!?]{0,40}(?:−\s*15\s*%|-\s*15\s*%|15\s*%)[^.!?]*/gi,
      alt
    )
    .replace(/примерно\s+на\s+15\s*%[^.!?]*/gi, 'сопровождал релизы и мониторинг на рабочем контуре')
    .replace(/−\s*15\s*%|-\s*15\s*%/gi, 'на рабочем контуре')
    .replace(/\s{2,}/g, ' ')
    .trim();
  if (out.length < t.length) {
    const pad = ' Готов обсудить стек и приоритеты команды.';
    while (out.length < t.length) {
      out = `${out}${pad}`.replace(/\s{2,}/g, ' ').trim();
      if (out.length >= t.length) break;
      // safety
      if (out.length > t.length + 200) break;
    }
  }
  return out;
}
