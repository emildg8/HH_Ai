...
import { HH_APPLY_EXIT_QUESTIONNAIRE_DEFERRED } from './hh-apply-exit-codes.mjs';
...
/**
 * Интерпретировать структурированный результат дочернего hh-apply-chat.
 * @param {{ exitCode?: number, skipReason?: string } | number} result
 */
export function interpretBatchApplyChildResult(result) {
  const rawExitCode =
    result && typeof result === 'object' ? result.exitCode : result;
  const parsedExitCode = Number(rawExitCode);
  const exitCode = Number.isInteger(parsedExitCode) ? parsedExitCode : 1;
  const skipReason =
    result && typeof result === 'object' ? String(result.skipReason || '').trim() : '';

  if (exitCode === 0) return { status: 'success', exitCode, skipReason };
  if (exitCode === HH_APPLY_EXIT_QUESTIONNAIRE_DEFERRED) {
    return { status: 'questionnaire', exitCode, skipReason };
  }
  ...
  return {
    ...
    exitCode,
    skipReason,
    message: `hh-apply-chat exit ${exitCode}${skipReason ? `: ${skipReason}` : ''}`,
  };
}

/** Ошибки отклика, при которых батч может безопасно перейти к следующей вакансии. */
export function isBatchRecoverableApplyError(message) {
  const text = String(message || '');
  if (/редирект на логин|нужен вход|npm run login/i.test(text)) return false;
  return (
    /отклик недоступен|в архиве|снята с публикации/i.test(text) ||
    /не удалось открыть форму отклика/i.test(text) ||
    /кнопка «откликнуться»/i.test(text) ||
    /не найдена кнопка «откликнуться»/i.test(text) ||
    /мастер не дошёл до кнопки/i.test(text) ||
    /не выбрано резюме|резюме не переключилось/i.test(text) ||
    /hh-apply-chat exit 1/i.test(text)
  );
}
