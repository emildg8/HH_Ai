...
export async function selectProfileResumeInResponseModal(page, opts = {}) {
  ...
  /** При поиске по названию не подставляем устаревший hash (часто ведёт на другое резюме). */
  const hash = preferredRaw ? '' : configHash;
  if (!preferredRaw && !hash) return null;

  if (preferredRaw) {
    const byTitle = await selectBestResumeByTitleInList(page, preferredRaw, '');
