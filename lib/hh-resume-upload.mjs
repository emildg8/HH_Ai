...
export async function readCurrentResponseResumeTitle(page) {
  ...
  const checkedRadios = root.locator('input[type="radio"]:checked');
  const checkedCount = await checkedRadios.count().catch(() => 0);
  for (let i = 0; i < checkedCount; i++) {
    const row = checkedRadios
      .nth(i)
      .locator('xpath=ancestor::*[.//*[@data-qa="resume-title"]][1]');
    const checkedTitle = row.locator('[data-qa="resume-title"]').first();
    if (await checkedTitle.isVisible({ timeout: 400 }).catch(() => false)) {
      return resumeItemTitle(checkedTitle);
    }
  }

  const selectedItem = root
    .locator(
      '[data-qa="resume-select-item"][class*="selected" i], [data-qa="resume-select-item"][aria-checked="true"]'
    )
    .first();
  if (await selectedItem.isVisible({ timeout: 400 }).catch(() => false)) {
    const selectedTitle = selectedItem.locator('[data-qa="resume-title"]').first();
    if (await selectedTitle.isVisible({ timeout: 400 }).catch(() => false)) {
      return resumeItemTitle(selectedTitle);
    }
  }
...
export async function ensurePreferredProfileResume(page, opts = {}) {
  ...
  if (
    !titleMatchesPreferred(afterTitle, preferredRaw) &&
    effectiveHash &&
