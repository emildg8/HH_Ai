...
export async function clickVacancyResponseButton(page, stepTimeoutMs = 12_000, opts = {}) {
  if (/applicant\/vacancy_response/i.test(page.url())) {
    const onForm =
      (await page.getByText(/отклик на вакансию|резюме для отклика/i).first().isVisible({ timeout: 500 }).catch(() => false)) ||
      (await page.locator('[data-qa="resume-select-item"]').first().isVisible({ timeout: 500 }).catch(() => false));
    if (onForm) return 'already-on-response-url';
    const { detectEmployerQuestionnaire } = await import('./hh-employer-questionnaire.mjs');
    const q = await detectEmployerQuestionnaire(page);
    if (q.detected && !q.captchaBlocking) return 'questionnaire-open';
  }

  const human = opts.humanClicks !== false;
  const pageForHuman = human ? page : null;
  const attemptMs = Math.min(stepTimeoutMs, 6000);

  const tryClick = async (locator, label) => {
    await clickVisibleFirst(locator, attemptMs, pageForHuman);
    return label;
  };
