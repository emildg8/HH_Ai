...
export function createZipFromDir(sourceDir, zipPath) {
  ...
  if (process.platform === 'win32') {
    const ps = [
      '-NoProfile',
      '-Command',
      `Compress-Archive -Path '${src.replace(/'/g, "''")}\\*' -DestinationPath '${dest.replace(/'/g, "''")}' -CompressionLevel Optimal -Force`,
    ];
    const r = spawnSync('powershell', ps, { stdio: 'inherit', shell: false });
    if (r.status !== 0) throw new Error(`Compress-Archive failed (${r.status})`);
    if (!fs.existsSync(dest)) throw new Error(`Archive not created: ${dest}`);
    return dest;
  }
