...
/**
 * @param {import('playwright').Page} page
 * @param {{ log?: (s: string) => void, skipCaptchaWait?: boolean, captchaContext?: string } | undefined} [opts]
 */
export async function assertHhLoggedIn(page, opts = {}) {
  let activePage = page;
  if (!opts.skipCaptchaWait) {
    const { ensureNoCaptchaBlocking } = await import('./hh-captcha-wait.mjs');
    const cap = await ensureNoCaptchaBlocking(activePage, {
      log: opts.log,
      context: opts.captchaContext || 'проверка доступа',
    });
    activePage = cap.page;
  }
  page = activePage;
  if (looksLikeLoginUrl(page.url())) {
    throw new Error(
      `Редирект на страницу входа hh.ru (url=${page.url()}). Выполните: npm run login`
    );
  }
  if (await isLoggedInOnHh(page)) return page;
  const url = page.url();
  throw new Error(
