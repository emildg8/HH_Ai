...
export function initBatchControl(params) {
  ...
  const resume = params.resumed ? readBatchResumeState() : null;
  writeBatchControl({
    command: 'running',
    ...
    params: { ...params },
    done: Number(resume?.done ?? 0) || 0,
    failed: Number(resume?.failed ?? 0) || 0,
    skipped: Number(resume?.skipped ?? 0) || 0,
    letterIdx: params.letterIdx ?? resume?.letterIdx ?? 0,
    processedIds: Array.isArray(resume?.processedIds) ? [...resume.processedIds] : [],
...
export function requestBatchResume() {
  const cmd = getBatchCommand();
  if (cmd === 'paused') setBatchCommand('running');
}
...
export function finishBatchControl({
  reason,
  done,
  failed,
  skipped,
  ...
  const c = readBatchControl();
  const finalLetterIdx = letterIdx ?? c?.letterIdx ?? 0;
  const finalProcessedIds = Array.isArray(processedIds)
    ? [...processedIds]
    : Array.isArray(c?.processedIds)
      ? [...c.processedIds]
      : [];
  if (reason === 'stop' || reason === 'pause') {
    persistBatchForResume({
      ...
      failed,
      skipped,
      letterIdx: finalLetterIdx,
      processedIds: finalProcessedIds,
      planned,
      reason,
    });
