...
export async function detectHhCaptcha(page) {
  ...
  try {
    ...
    // hh после решения капчи дописывает ?hhtmFrom=account_captcha — это не капча.
    let urlForCheck = url;
    try {
      const u = new URL(url);
      urlForCheck = u.origin + u.pathname;
    } catch {}
    if (CAPTCHA_URL_RE.test(urlForCheck)) hintSet.add('url');
    ...
    if (/captcha|доступ ограничен|проверка безопасности|\bare you a robot\b/i.test(title))
      hintSet.add('title');
