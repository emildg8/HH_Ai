...
/**
 * Политика preflight: резюме уже clients + баннер на форме → не STOP (ложный Magritte).
 * @param {{ resumeLooksClients: boolean, formVisibilityBlocked: boolean }} input
 * @returns {{ ok: boolean, formBannerIgnored?: boolean, reason?: string, message?: string }}
 */
export function decideClientsFormBannerPolicy(input) {
  const resumeLooksClients = Boolean(input?.resumeLooksClients);
  const formVisibilityBlocked = Boolean(input?.formVisibilityBlocked);
  if (resumeLooksClients && formVisibilityBlocked) {
    return {
      ok: true,
      formBannerIgnored: true,
      reason: 'false_positive_already',
      message:
        'Резюме уже видно работодателям; баннер Magritte на форме — ложный, идём в отклик',
    };
  }
  if (formVisibilityBlocked) {
    return {
      ok: false,
...
async function showResumeVisibleToHhClientsViaEditPage(page, hash, opts = {}) {
  ...
  if (HH_CLIENTS_HINT.test(body)) {
    ...
    if (verifyVid) {
      ...
      if (formBlock?.blocked) {
        const policy = decideClientsFormBannerPolicy({
          resumeLooksClients: true,
          formVisibilityBlocked: true,
        });
        log(
          '[resume-visibility] Резюме уже clients, баннер на форме игнор (formBannerIgnored) — продолжаем отклик'
        );
        return { ...policy, already: true, formBlock };
      }
    }
  ...
  if (opts.verifyVacancyId) {
    ...
    // Серая кнопка ≠ visibility: письмо/анкета тоже дизейблят submit. Только явный баннер.
    if (/поменяйте\s+видимост/i.test(bodyAfter)) {
      ...
      return {
        ...
        message: 'После смены видимости форма отклика всё ещё требует «поменяйте видимость»',
        options,
      };
    }
  }
  ...
  if (HH_CLIENTS_HINT.test(cardText)) {
    ...
    if (verifyVid) {
      ...
      if (formBlock?.blocked) {
        ...
        log(
          '[resume-visibility] Список: clients уже ок; баннер на форме игнор (formBannerIgnored)'
        );
        return { ...policy, already: true, formBlock };
      }
    }
...
export async function detectResumeVisibilityBlockOnResponseForm(page) {
  ...
  // Magritte 07.2026: UI = «всем работодателям на hh.ru», форма ещё пишет «компаниям-клиентам».
  // Если резюме уже clients — баннер ложный; явный обход: HH_RESUME_VISIBILITY_FORM_SKIP=1
  if (String(process.env.HH_RESUME_VISIBILITY_FORM_SKIP ?? '').trim() === '1') {
    return null;
  }
