...
/**
 * После клика считаем отклик отправленным только по явному подтверждению hh.ru.
 * Закрытие формы само по себе может быть редиректом на логин или ошибкой сервера.
 * @param {{ url?: string, confirmed?: boolean }} state
 * @returns {'confirmed'|'session-expired-after-submit'|'no-confirmation-after-submit'}
 */
export function classifyResponseSubmission(state = {}) {
  if (looksLikeLoginUrl(state.url || '')) return 'session-expired-after-submit';
  if (state.confirmed) return 'confirmed';
  return 'no-confirmation-after-submit';
}
...
export async function completeVacancyResponseForm(page, opts = {}) {
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    if (submit && !pendingLetter && !pendingResume) {
      ...
      const submissionState = classifyResponseSubmission({
        url: page.url(),
        confirmed: await isResponseAlreadySubmitted(page),
      });
      ...
      log(
        submissionState === 'session-expired-after-submit'
          ? '[hh-response-modal] После клика сессия hh.ru истекла — отклик не подтверждён'
          : `[hh-response-modal] Клик «${submit.label}», но hh.ru не подтвердил отправку`
      );
      return {
        submitted: false,
