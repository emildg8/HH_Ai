...
export async function completeVacancyResponseForm(page, opts = {}) {
  ...
  if (needResume) {
    if (await isEmployerQuestionnaireWizardStep(page)) {
      // Анкета уже после sticky резюме — сверка двусторонняя (Альтуэра + Биржа 18.07 + P2 infra 20.07).
      // Один reload+hash + sync, если gate.block или sticky devops↔infra (даже до hard STOP).
      const {
        questionnaireStepResumeBlocksSubmit,
        questionnaireInfraFamilyStickyMismatch,
      } = await import('./hh-resume-picker.mjs');
      const ideal = opts.resumeTarget?.role || '';
      let curTitle = await readCurrentResponseResumeTitle(page).catch(() => '');
      let gate = questionnaireStepResumeBlocksSubmit({
        curTitle,
        preferredTitle,
        idealRole: ideal,
      });
      let stickyInfraFamily = questionnaireInfraFamilyStickyMismatch(ideal, curTitle);
      const needsStickyRepair = Boolean(gate.block || stickyInfraFamily.mismatch);
      if (needsStickyRepair && resumeHash && vacancyId) {
        log(
          `[hh-response-modal] Sticky «${curTitle || '—'}» при ideal «${preferredTitle || ideal}» — reload+resumeId ×1`
        );
        await reloadVacancyResponseWithResume(page, { vacancyId, resumeHash, log });
        await page.waitForTimeout(isFastMode() ? 300 : 900);
        curTitle = await readCurrentResponseResumeTitle(page).catch(() => '');
        gate = questionnaireStepResumeBlocksSubmit({
          curTitle,
          preferredTitle,
          idealRole: ideal,
        });
        stickyInfraFamily = questionnaireInfraFamilyStickyMismatch(ideal, curTitle);
      }
      if ((gate.block || stickyInfraFamily.mismatch) && needResume) {
        log(`[hh-response-modal] Sticky после reload — syncPreferredResume / picker ×1`);
      ...
      if (gate.block || stickyInfraFamily.mismatch) {
        const stopReason = gate.block ? gate.reason : stickyInfraFamily.reason;
        log(
          `[hh-response-modal] STOP на шаге анкеты: в форме «${curTitle || '—'}», нужно «${preferredTitle || ideal}» (${stopReason})`
        );
        return {
          submitted: false,
          letterInForm: false,
...

  {
    ...
    if (visBlock?.blocked) {
      ...
      // Magritte: на форме часто ссылка «поменяйте видимость» — один клик, без ухода с отклика
      const fixVis = page
        .getByRole('link', { name: /поменяйте\s+видимост|изменить\s+видимост/i })
        .or(page.getByRole('button', { name: /поменяйте\s+видимост|изменить\s+видимост/i }))
        .or(page.locator('a, button').filter({ hasText: /поменяйте\s+видимост|изменить\s+видимост/i }))
        .first();
      if (await fixVis.isVisible({ timeout: 2500 }).catch(() => false)) {
        log('[hh-response-modal] Клик по «поменяйте видимость» на форме отклика');
        await fixVis.click({ force: true });
        await page.waitForTimeout(1500);
        const clients = page
          .locator('[data-qa="resume-visibility-card-access-type-clients"]')
          .or(page.getByRole('radio', { name: /всем\s+работодателям|компаниям[-\s]*клиентам/i }))
          .first();
        if (await clients.isVisible({ timeout: 3000 }).catch(() => false)) {
          await clients.click({ force: true });
          await page.waitForTimeout(400);
          const save = page.getByRole('button', { name: /^(сохранить|подтвердить|готово|ок)$/i }).first();
          if (await save.isVisible({ timeout: 2000 }).catch(() => false)) {
            await save.click({ force: true });
            await page.waitForTimeout(1800);
          }
        }
        const still = await detectResumeVisibilityBlockOnResponseForm(page);
        if (!still?.blocked) {
          log('[hh-response-modal] Видимость резюме на форме снята — продолжаем отклик');
        } else {
          return {
            submitted: false,
            letterInForm: false,
