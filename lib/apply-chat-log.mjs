...
/** Без метки времени — для сравнения с последней строкой файла (не дублировать параллельные записи). */
function logBodyKey(line) {
  return String(line ?? '')
    .trim()
...
export function appendApplyChatLog(line, opts = {}) {
  ...
  if (opts.dedupe !== false && fs.existsSync(HH_APPLY_CHAT_LOG_FILE)) {
    try {
      const tail = fs.readFileSync(HH_APPLY_CHAT_LOG_FILE, 'utf8').slice(-8192);
      const last = tail
        .split('\n')
        .map((l) => l.trim())
        .filter(Boolean)
        .pop();
      if (last && logBodyKey(last) === logBodyKey(text)) return;
    } catch {
      /* ignore */
    }
  }
