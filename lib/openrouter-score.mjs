...
/** Запасные free-модели, если openrouter/free отдал 404 у провайдера (Nvidia и т.п.). */
export function openRouterModelFallbackChain() {
  const primary = resolveOpenRouterModelForRequest();
  const raw = (process.env.OPENROUTER_MODEL_FALLBACKS || '').trim();
  const extras = raw
...
export function isOpenRouterModelUnavailableError(err) {
  const msg = String(err?.message ?? err);
  return /\bLLM\s+404\b/i.test(msg) || /not found|no endpoints|provider returned error/i.test(msg);
}
...
function buildUserPrompt(vacancy, cvBundle, prefs, feedbackBlock) {
  const targetRub = prefs?.targetMonthlyRub ?? 180000;
  const minRub = prefs?.minMonthlyRub ?? 150000;
  return `Ты помощник одного соискателя. Он сам решает, на какие вакансии откликаться. У него ДВЕ версии резюме ниже — ОБЕ его, под разные акценты (не два разных человека).
Целевые роли: DevOps / SRE / платформа (junior+ / middle, НЕ главный/chief/lead DevOps) И техподдержка L2/L3 / системный инженер / TAM — L2≈middle, L3≈senior в поддержке это нормально. L1/helpdesk-only — низкий балл.
Зарплата: ориентир в резюме ~${targetRub} ₽/мес, минимум ~${minRub} ₽; вилка до ~300k ₽ или без ЗП — ок. Ниже минимума без внятной причины — штраф к scoreVacancy.
Формат: приоритет удалёнка по РФ (МСК UTC+3 или до +2ч от МСК); гибрид/офис только Москва; ночные смены и обязательный разговорный английский — штраф. Дежурства, графики 5-2 / 2-2 / 4-3 — допустимы.
Жёсткие фильтры скрипта уже отсеяли явный мусор; твоя задача — тонкая релевантность и CV.
${feedbackBlock}
Оцени вакансию: стоит ли тратить время на отклик.

Смысл полей scoreVacancy / scoreCvMatch / scoreOverall — целые от 0 до 100:
- scoreVacancy: роль и уровень, формат работы, ЗП, график, язык, рост в DevOps/AI, красные флаги (без построчной сверки CV).
- scoreCvMatch: перекрытие обоих резюме с требованиями; обоснованность отклика.
- scoreOverall: итог (вес vacancy ~45%, cvMatch ~55% — если сомневаешься, согласуй с этим).
