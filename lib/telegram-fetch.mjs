 ...
 */
export async function telegramFetch(urlStr, init = {}, timeoutMs = 35000) {
  ...
  if (!timeoutMs || timeoutMs <= 0) return doFetch();
  return Promise.race([
    doFetch(),
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error(`Telegram fetch timeout (${timeoutMs}ms)`)), timeoutMs)
    ),
  ]);
}
