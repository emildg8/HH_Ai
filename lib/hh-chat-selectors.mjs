...
export async function verifyLetterVisibleInChat(page, expectedText) {
  ...
  const probe = raw.slice(0, Math.min(72, raw.length)).trim();
  if (probe.length < 28) return false;

  const scan = async (frame) => {
    try {
      return await frame.evaluate((expectedProbe) => {
        const clone = document.body?.cloneNode(true);
        if (!clone) return false;
        clone
          .querySelectorAll(
            'textarea, input, select, [contenteditable="true"], [contenteditable=""], [role="textbox"]'
          )
          .forEach((node) => node.remove());
        const text = (clone.innerText || clone.textContent || '').replace(/\s+/g, ' ');
        return text.includes(expectedProbe);
      }, probe);
    } catch {
      return false;
    }
...
export async function waitForLetterVisibleInChat(page, expectedText, timeoutMs = 6000) {
  const deadline = Date.now() + Math.max(0, Number(timeoutMs) || 0);
  do {
    if (await verifyLetterVisibleInChat(page, expectedText)) return true;
...
async function sendLetterInRoot(root, page, opts, timeoutMs, allowPageFallback = false) {
  const { text, humanTyping = false } = opts;
  await page.waitForTimeout(800);
  ...
  const tryFill = async (locator, label, stepMs = timeoutMs) => {
    ...
    if (await waitForLetterVisibleInChat(page, text, fast ? 2500 : 6000)) {
      logFn(`[hh-chat] Письмо в переписке подтверждено (${label})`);
      return label;
    }
    const err = new Error(`Отправка письма в чате не подтверждена (${label})`);
    err.code = 'CHAT_DELIVERY_UNCONFIRMED';
    throw err;
  };
  ...
  for (const run of fillAttempts) {
    try {
      ...
      if (e?.code === 'CHAT_DELIVERY_UNCONFIRMED') throw e;
      lastErr = e;
    }
  }
