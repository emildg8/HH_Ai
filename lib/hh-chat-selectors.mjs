...
function withoutCompanyLegalForm(value) {
  return normalizedIdentityText(value)
    .split(' ')
    .filter((token) => !/^(ооо|ао|пао|зао|ип|компания)$/i.test(token))
    .join(' ');
}
...
export function negotiationThreadTextMatches(text, vacancyTitle, company) {
  ...
  const companyText = withoutCompanyLegalForm(company);
  if (title.length < 3 || !companyText) return false;

  const lines = String(text || '')
    .split(/\r?\n/)
    .map((line) => normalizedIdentityText(line))
    .filter(Boolean);
  const titleMatches = lines.some((line) => line === title);
  const companyMatches = lines.some((line) => withoutCompanyLegalForm(line) === companyText);
