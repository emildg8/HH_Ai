...
function vacancyIdFromHref(href) {
  return String(href || '').match(/(?:\/vacancy\/|[?&]vacancyId=)(\d+)/i)?.[1] || '';
}

/**
 * Текст строки переговоров должен подтверждать и вакансию, и работодателя.
 * Совпадения по одному общему слову (например, DevOps) недостаточно.
 */
export function negotiationThreadTextMatches(text, vacancyTitle, company) {
  const haystack = normalizedIdentityText(text);
  const title = normalizedIdentityText(vacancyTitle);
  const companyText = normalizedIdentityText(company);
  if (!haystack || title.length < 3 || !companyText) return false;

  const titleTokens = title.split(' ').filter((token) => token.length > 2);
  const companyTokens = companyText
    .split(' ')
    .filter((token) => token.length > 2 && !/^(ооо|пао|зао|компания)$/i.test(token));
  const titleMatches =
    haystack.includes(title) ||
    titleTokens.filter((token) => haystack.includes(token)).length >= Math.min(2, titleTokens.length);
  const companyMatches =
    haystack.includes(companyText) || companyTokens.some((token) => haystack.includes(token));
...
export async function selectNegotiationThread(page, ctx = {}) {
  ...
  if (vacancyId) {
    const byVacancy = page.locator(
      `a[href*="/vacancy/${vacancyId}"], a[href*="vacancyId=${vacancyId}"]`
    );
    const vacancyLinks = await byVacancy.count().catch(() => 0);
    for (let i = 0; i < vacancyLinks; i++) {
      const link = byVacancy.nth(i);
      const href = await link.getAttribute('href').catch(() => '');
      if (vacancyIdFromHref(href) !== vacancyId) continue;
      if (await link.isVisible({ timeout: 2200 }).catch(() => false)) {
  ...
  for (let i = 0; i < Math.min(n, 30); i++) {
    ...
    const rowVacancyId = vacancyIdFromHref(await row.getAttribute('href').catch(() => ''));
    if (vacancyId && rowVacancyId && rowVacancyId !== vacancyId) continue;
    const text = await row.innerText().catch(() => '');
    if (negotiationThreadTextMatches(text, ctx.vacancyTitle, ctx.company)) matches.push(row);
  }
  if (matches.length === 1) {
    await matches[0].scrollIntoViewIfNeeded().catch(() => {});
    await matches[0].click();
    await page.waitForTimeout(1400);
    log(`[hh-chat] Диалог подтверждён по вакансии и работодателю`);
    return 'thread:identity';
  }
  if (matches.length > 1) {
    log('[hh-chat] Найдено несколько похожих диалогов — автоматическая отправка отменена');
  }
...
export async function openEmployerChatAfterResponse(pageIn, ctx = {}, timeoutMs = 45_000) {
  ...
  const negotiationsListUrl = /\/applicant\/negotiations(?:[/?#]|$)/i;
  const onNegotiationsList = () => negotiationsListUrl.test(page.url());

  const ensureThreadOpen = async (source) => {
    if (onNegotiationsList()) {
    ...
    return (await isChatInputReady(page)) ? source : null;
  };
...
export async function verifyLetterVisibleInChat(page, expectedText) {
  ...
  for (const len of [100, 60, 36]) {
    const s = raw.slice(0, len).trim();
    if (s.length >= 20) needles.add(s);
  }
  const tail = raw.length > 80 ? raw.slice(-50).trim() : '';
  if (tail.length >= 20) needles.add(tail);
  for (const m of raw.matchAll(/\b(DevOps|Docker|Kubernetes|мониторинг|Terraform|Ansible)\b/gi)) {
    if (m[0].length >= 4) needles.add(m[0]);
  }
  ...
  const scan = async (frame) => {
    try {
      return await frame.evaluate((snippets) => {
...
async function sendLetterInRoot(root, page, opts, timeoutMs, allowPageFallback = false) {
  ...
  const tryFill = async (locator, label, stepMs = timeoutMs) => {
    ...
    const inField = await fieldContainsExpectedText(locator, text);
    if (inField) {
...
export async function sendLetterInChat(page, opts, timeoutMs = 25_000) {
  ...
  if (/\/applicant\/negotiations/i.test(page.url())) {
    ...
    if (!picked) {
      throw new Error(
        'Не удалось открыть чат: целевой диалог не подтверждён по вакансии и работодателю.'
      );
    }
    log(`[hh-chat] Перед вводом: ${picked}`);
  ...
  if (await chatIframeVisible(page)) {
    ...
    try {
      ...
      if (await iframeInput.count().catch(() => 0)) {
        try {
          await iframeInput.click({ force: true, timeout: 8000 });
