...
import { enqueueFromShipOutcomes } from './hh-chat-ai-queue.mjs';
import {
...
import { pruneAnastasiaDayBasketSummary } from './anastasia-day-basket-pick.mjs';
 ...
 * Битые id (нет в store после corrupt/reharvest) отбрасываются и вычищаются из summary.
 * @param {{ onlyId?: string, prune?: boolean }} [opts]
 * @returns {string[]}
 */
export function loadAnastasiaDayBasketIds(opts = {}) {
  if (opts.onlyId) {
    const id = String(opts.onlyId);
    return getVacancyRecord(id) ? [id] : [];
  }
  const dayPath = path.join(logsDir(), 'anastasia-day-baskets-summary.json');
  if (opts.prune !== false) {
    const pruned = pruneAnastasiaDayBasketSummary(dayPath);
    if (pruned.rewritten) {
      console.warn(
        `[hunt-day] day-baskets: убраны id вне store (${pruned.removed.length}): ${pruned.removed
          .map((x) => x.slice(0, 8))
          .join(', ')}`
      );
    }
  ...
  try {
    ...
    return (day.baskets || [])
      ...
      .filter(Boolean)
      .map(String)
      .filter((id) => Boolean(getVacancyRecord(id)));
  } catch {
    return [];
  }
}
 ...
 * @param {boolean} [opts.noSync]
 * @param {boolean} [opts.skipNegotiationsPrep]
 */
 ...
 * @param {boolean} [opts.forceOnly] — обход hard site-preflight для --only=
 * @param {(script: string, args: string[]) => Promise<number>} [opts.spawnChild]
...
export async function runHuntDayShip(opts = {}) {
  ...
  try {
    if (go && !dryRun) {
      const { prepareHuntDayNegotiations } = await import('./hunt-day-negotiations-prep.mjs');
      if (opts.noSync === true && opts.forceNoSync !== true) {
        throw new Error(
          'hunt-day ship --go: нужен sync переговоров. Уберите --no-sync или задайте --force-no-sync только для отладки.'
        );
      }
      negotiationsPrep = prepareHuntDayNegotiations({
        sync: opts.sync === true,
        noSync: opts.forceNoSync === true,
        log: console.log,
      });
    }
    if (mode === 'point') {
      ...
      if (opts.forceOnly) args.push('--force-only');
...
/**
 * После успешного ship — очередь ИИ-помощника (/chat), пауза под появление бота, затем process --go.
 * Бот часто пишет в первую минуту: default wait 25s + повтор через 25s.
 *
 * @param {object} opts
 * @param {object[]} opts.outcomes
 * @param {number} [opts.waitSec=25]
 * @param {boolean} [opts.processGo=true] — false = только enqueue (старое поведение)
 * @param {number} [opts.maxTurns=6]
 * @param {(script: string, args: string[]) => Promise<number>} [opts.spawnChild]
 * @param {(msg: string) => void} [opts.log]
 */
export async function runHuntDayAfterShipChatAi(opts = {}) {
  const log = opts.log || console.log;
  const spawnChild = opts.spawnChild || spawnHuntDayChild;
  const waitSec = Math.max(0, Number(opts.waitSec ?? 25));
  const processGo = opts.processGo !== false;
  const maxTurns = Math.max(1, Number(opts.maxTurns ?? 6));
  const results = enqueueFromShipOutcomes(opts.outcomes || []);
  const enqueued = results.filter((r) => r.enqueued).length;
  log(`[hunt-day] after-ship=chat-ai: enqueued=${enqueued} / ${results.length}`);

  /** @type {number[]} */
  const processExits = [];
  if (processGo && enqueued > 0) {
    const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
    const runProcess = async (pass) => {
      log(`[hunt-day] after-ship=chat-ai: process --go (#${pass})`);
      const code = await spawnChild('scripts/devops-hh-chat-ai-queue.mjs', [
        'process',
        '--go',
        `--max-turns=${maxTurns}`,
      ]);
    ...
    if (waitSec > 0) {
      log(`[hunt-day] after-ship=chat-ai: ждём ${waitSec}s появления ИИ-помощника…`);
      await sleep(waitSec * 1000);
    }
    await runProcess(1);
    // второй проход — бот часто добивает follow-up за <1 мин
    const secondWait = Math.min(30, Math.max(15, waitSec));
    log(`[hunt-day] after-ship=chat-ai: ждём ещё ${secondWait}s (follow-up)…`);
    await sleep(secondWait * 1000);
    await runProcess(2);
  } else if (processGo && enqueued === 0) {
    log('[hunt-day] after-ship=chat-ai: нечего process (enqueued=0)');
  } else if (!processGo) {
    log('[hunt-day] дальше: npm run devops:hh-chat-ai-queue -- process --go');
  }

  const payload = {
    at: new Date().toISOString(),
    command: 'after-ship-chat-ai',
    enqueued,
    waitSec,
    processGo,
    processExits,
  ...
  payload.outPath = writeHuntDayJson('hunt-day-after-ship-chat-ai-latest.json', payload);
  return payload;
}
