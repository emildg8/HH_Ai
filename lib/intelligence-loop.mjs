...
export function writeIntelligenceBaseline() {
  const digest = runIntelligenceLoop({ label: 'baseline', writeDigest: false });
  fs.mkdirSync(DATA_DIR, { recursive: true });
  fs.writeFileSync(INTELLIGENCE_BASELINE_FILE, `${JSON.stringify(digest, null, 2)}\n`, 'utf8');
  return digest;
}
