...
export const EMPLOYMENT_CANON = {
  ...
  softline: {
    ...
    stackLine:
      'vSphere, vSAN, Veeam, Zabbix, Jira, Confluence, Nexus, SonarQube, Windows Server, AD, DNS/DHCP, VM, SLA/OLA, ITIL-практики',
  },
  fbd: {
    company: 'ФБД',
...
export function buildItOneSbpEntry(trackId) {
  ...
  if (id === 'devops') {
    lines = [
      'На проекте ДПСИТ (кредитный контур банка, 24/7): сопровождал релизы в OpenShift, процессы Camunda, доступ Keycloak, диагностику по SQL и логам.',
    ...
    lines = [
      'На проекте ДПСИТ: кредитный контур 24/7 — заявки по продуктам, инциденты, SQL, разбор по логам; OpenShift/Camunda/Keycloak в контуре.',
      ...
      'Участвовал в релизах и регламентных работах: согласование окон, контроль рисков, документация для дежурных.',
    ];
  } else if (id === 'tam') {
    lines = [
    ...
    lines = [
      'На проекте ДПСИТ: сопровождение кредитного контура 24/7 — распределение нагрузки смены, приоритеты, эскалации, контроль SLA.',
...
export function buildSoftlineBlock(trackId) {
  ...
  if (id === 'support_lead') {
    lines = [
      ...
      'Обеспечивал инфраструктуру ТП на площадках RackStore (Москва) и Уфа: vSphere/vSAN (около 10 хостов, крупный парк ВМ), жизненный цикл ВМ (заявка в Jira → шаблон → домен → резервное копирование → Confluence), AD/DNS/DHCP.',
      'Перевёз офис и серверы из офиса в ЦОД без простоя ключевых сервисов; участвовал в инцидентах vSAN и восстановлении между площадками.',
      'Внедрил Atlassian-стек (Jira, Confluence) и практики SCM (Nexus, SonarQube); планировал бюджет, лицензии и взаимодействие с подрядчиками и ИБ.',
    ];
  } else if (id === 'infra') {
    lines = [
      'Аплана → Softline (май 2021 — июль 2023): сопровождал инфраструктуру на площадках RackStore (Москва) и Уфа: vSphere/vSAN (около 10 хостов, крупный парк ВМ), два ЦОДа, Veeam, Zabbix, AD/DNS/DHCP.',
      'Вёл жизненный цикл виртуальных машин: заявка в Jira → шаблон → домен → задание резервного копирования → описание в Confluence; доступ снаружи через reverse-proxy/VPN, без прямого RDP/SSH.',
      'Перевёз офис и серверы из офиса в ЦОД; участвовал в инцидентах vSAN и аварийном восстановлении сервисов между площадками после отказа физсервера.',
      'Параллельно руководил службой более 20 человек: SLA/OLA (дуга ~78%→~93%), связь с Service Desk ГК; Jira/Confluence, учёт оборудования.',
    ];
  } else if (id === 'l2l3') {
    lines = [
      'Аплана → Softline (май 2021 — июль 2023): в контуре ЦСИ — Service Desk / Jira, эскалации L2–L3 по критичности, терминальный доступ (AD, печать, SCCM), MFA.',
      'Вёл сложные эскалации и базу знаний; обучал L2 по типовым сценариям.',
      'Внедрил и сопровождал Atlassian-стек (Jira, Confluence); перевёз офис и серверы в ЦОД без простоя ключевых сервисов.',
      'В службе поддержки более 20 человек отвечал за регламенты ЦСИ и качество разбора сложных обращений (не только L1).',
    ];
  } else if (id === 'tam') {
    lines = [
      'Аплана → Softline (май 2021 — июль 2023): выстраивал коммуникацию с заказчиками и отчётность по SLA; планировал бюджет подразделения.',
      'Внедрил Atlassian-стек (Jira, Confluence) и практики SCM (Nexus, SonarQube).',
      'Перевёз офис и серверы из офиса в ЦОД; сократил поток типовых обращений клиентов более чем на порядок за счёт базы знаний и процессов.',
    ...
    lines = [
      ...
      'Перевёз офис и серверы из офиса в ЦОД без простоя ключевых сервисов.',
...
export const STYLE_GUIDE_RU = {
  ...
  softlineNas:
    'Softline/Аплана: RackStore+Уфа, vSphere/vSAN, жизненный цикл ВМ, переезд офиса/серверов в ЦОД; на IC-резюме руководство не первым буллетом.',
  fbdSameEmployer:
