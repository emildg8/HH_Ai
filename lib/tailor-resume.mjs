...
function readBaseResumeMd() {
  const files = fs.existsSync(CV_DIR)
    ? fs.readdirSync(CV_DIR).filter((n) => n.endsWith('.md') && !n.startsWith('.')).sort()
    : [];
  if (!files.length) throw new Error('Нет CV/*.md — положите резюме в папку CV/');
  return fs.readFileSync(path.join(CV_DIR, files[0]), 'utf8');
}
...
export function buildTailoredResumeMarkdown(vacancy) {
  ...
  const merged = [...new Set([...mustHave, ...skills])].slice(0, 10);
  ...
  const header = [
    ...
    ``,
    skills.length
      ? `В вакансии упоминаются: ${merged.join(', ')}. У меня практический опыт по этим направлениям (банковский прод, L2/инфраструктура, мониторинг, релизы, Docker/OpenShift).`
