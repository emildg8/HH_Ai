...
function readBaseResumeMd() {
  const preferred = path.join(CV_DIR, '01-devops-emil-shahvaladov.md');
  if (fs.existsSync(preferred)) return fs.readFileSync(preferred, 'utf8');
  const files = fs.existsSync(CV_DIR)
    ? fs.readdirSync(CV_DIR).filter((n) => n.endsWith('.md')).sort()
    : [];
  if (!files.length) throw new Error('Нет CV/*.md — сначала npm run cv:export');
